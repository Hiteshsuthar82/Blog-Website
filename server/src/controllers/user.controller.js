...
// upgrade the user
 const upgradeToPremium = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, user, "You are now a premium user!"));
});



// ONLY IMAGE UPLOAD AND GIVE URL
...
export const sendCustomEmail = asyncHandler(async (req, res) => {
  ...
  // Send custom email
  await sendEmail(email, "Custom Notification", "custom_email_template", {
    message: "This is a custom email notification.",
  });
